...
export function redisTransactionToInsert(tx: RedisTransaction): TransactionInsert {
  return {
    amount: tx.amount,
    fee: tx.fee,
...
export function createRedisTransaction(
  partial: Partial<RedisTransaction> & Pick<RedisTransaction, 'amount' | 'fee' | 'source_address_pubkey' | 'destination_address_pubkey' | 'transaction_type' | 'layer2_transaction_id' | 'signature'>,
): RedisTransaction {
  return {
