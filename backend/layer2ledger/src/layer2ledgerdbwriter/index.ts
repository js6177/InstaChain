...
while (true) {
  try {
    ...
    if (transactionsToProcess.length === 0 && withdrawalsToProcess.length === 0) {
    ...
    await db.transaction(async (tx) => {
      for (const pendingTx of transactionsToProcess) {
        ...
        await tx.insert(transactions).values(redisTransactionToInsert(pendingTx.transaction));
      ...
      for (const pendingWithdrawal of withdrawalsToProcess) {
        ...
        await tx.insert(withdrawalRequests).values({
          ...
          layer1TransactionId: pendingWithdrawal.withdrawal_request.layer1_transaction_id,
          status: pendingWithdrawal.withdrawal_request.status,
          amount: pendingWithdrawal.withdrawal_request.amount,
          layer2WithdrawalId: pendingWithdrawal.withdrawal_request.layer2_withdrawal_id,
          serverSignature: pendingWithdrawal.withdrawal_request.server_signature,
          layer2TransactionId: pendingWithdrawal.withdrawal_request.layer2_transaction_id,
    ...
    if (transactionsToProcess.length > 0) {
      await redis.ltrim(PENDING_TRANSACTIONS_LIST_KEY, transactionsToProcess.length, -1);
    }
    if (withdrawalsToProcess.length > 0) {
      await redis.ltrim(PENDING_WITHDRAWALS_LIST_KEY, withdrawalsToProcess.length, -1);
    }

    for (const pendingTx of transactionsToProcess) {
      if (pendingTx.lock_token) {
        await lockManager.releaseMultiLock(pendingTx.addresses_locked, pendingTx.lock_token);
      }
    }
    for (const pendingWithdrawal of withdrawalsToProcess) {
