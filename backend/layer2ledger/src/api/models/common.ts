...
const errorMessages: Record<number, string> = {
  ...
  [ErrorCodes.CANNOT_DUPLICATE_TRANSACTION]: 'Transaction with that ID (nonce) already exists',
  [ErrorCodes.INSUFFICIENT_FUNDS]: 'Insufficient funds',
  [ErrorCodes.TRANSACTION_ID_NOT_FOUND]: 'Cannot find the transaction',
  [ErrorCodes.DEPOSIT_ADDRESS_NOT_FOUND]: 'The address that funds were deposited to was not found',
  [ErrorCodes.DUPLICATE_TRANSACTION_ID]: 'The nonce must be unique',
  [ErrorCodes.ADDRESS_LOCKED]: 'The address is locked because it is being processed by another transaction. Please try again',
...
export function buildCommonResponse(errorCode: number, errorMessage?: string): CommonResponse {
  return {
