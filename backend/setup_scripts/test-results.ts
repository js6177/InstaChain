...
export function hostResultPath(outputDir: string, service: DockerServiceName): string {
 ...
 */
export function getServiceTestCommand(service: DockerServiceName): ServiceTestCommand {
...
export function ensureServiceResultFile(
  ...
  const caseName =
    service === DockerService.TEST_LAYER2LEDGER_OAUTH_MANAGER ? 'health' : 'seed';
...
export function parseJunitCounts(
  ...
  if (suites.length === 0) {
    ...
    if (root) {
      return countsFromAttributes(service, root[1] ?? '', containerPassed, true);
    }
    return {
      service,
      containerPassed,
...
export function printTestResultsSummary(
  ...
  for (const result of results) {
    ...
    const status = result.containerPassed && result.failed === 0 ? 'PASSED' : 'FAILED';
