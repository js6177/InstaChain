...
function generateLayer2LedgerOAuthManagerConfig(
  ...
  const configPath = getConfigFilePath(Services.LAYER2LEDGEROAUTHMANAGER, environment);
...
function loadBitcoinRpcSettingsForImport(
  ...
  if (!existsSync(confPath)) {
    throw new Error(`bitcoin.conf not found at ${confPath}. Run with -generate-keys first.`);
  }
...
function generateKeys(
  ...
  const layer2bridgeSigningAddress = new Layer2Address('', '', '', new Uint8Array(), new Uint8Array());
  layer2bridgeSigningAddress.generateNewAddress();
  const onboardingLayer2DepositAddress = new Layer2Address('', '', '', new Uint8Array(), new Uint8Array());
  ...
  const rpcHost = containered ? layer2ledgerDockerEnv.bitcoinRpcHost : 'localhost';
  ...
  const layer2bridgeSettings: Layer2BridgeConfig = {
    ...
    onboarding_signing_private_key: layer2bridgeSigningAddress.private_key_str_base58,
  };

  const commonBackendSettings: CommonBackendConfig = {
    node_id: layer2ledgerApihandlerSettings.layer2ledger_node_id,
    layer2bridge_signing_public_key: layer2bridgeSigningAddress.public_key_str_base58,
  };

  writeBitcoinConf(getConfigFilePath('bitcoin.conf', env), bitcoinConf);
  writeConfig(getConfigFilePath(Services.LAYER2LEDGER_COMMON, env), layer2ledgerCommonSettings);
  writeConfig(
  ...
  writeConfig(getConfigFilePath(Services.LAYER2LEDGERBRIDGE, env), layer2bridgeSettings);
  writeConfig(getConfigFilePath(Services.BACKEND_COMMON, env), commonBackendSettings);

  const tempKeysPath = getConfigFilePath(Intermediate.BITCOIN_CORE_MASTER_KEYS, env);
...
async function importKeysToBitcoinCore(
  ...
  if (resolvedBridgeSettings === null) {
    const bridgeSettingsPath = getConfigFilePath(Services.LAYER2LEDGERBRIDGE, env);
  ...
  const loadResp = await rpcClient.loadWallet(resolvedBridgeSettings.wallet_name);
  if (loadResp.error) {
    if (isWalletAlreadyLoaded(loadResp.error)) {
      console.log(`Wallet '${resolvedBridgeSettings.wallet_name}' is already loaded.`);
    } else {
      console.log(
        `Could not load wallet, attempting to create it. Error: ${loadResp.error.code} - ${loadResp.error.message}`,
      );
      const createResp = await rpcClient.createWallet(resolvedBridgeSettings.wallet_name);
      if (createResp.error) {
        if (isWalletAlreadyExists(createResp.error)) {
          ...
          const loadResp2 = await rpcClient.loadWallet(resolvedBridgeSettings.wallet_name);
        ...
        console.log(`Wallet '${resolvedBridgeSettings.wallet_name}' created successfully.`);
      }
    }
  } else {
    console.log(`Wallet '${resolvedBridgeSettings.wallet_name}' loaded successfully.`);
  }
  ...
  try {
    ...
    for (const [index, result] of results.entries()) {
      if (result.success) {
        ...
        console.log(`Failed to import descriptor ${index}: ${JSON.stringify(result.error)}`);
      }
    }
  } catch (error) {
...
async function main(): Promise<void> {
  ...
  const flag = (name: string): boolean =>
    argv.includes(`-${name}`) || argv.includes(`--${name}`) || Boolean(values[name as keyof typeof values]);
  ...
  const env = getArgValue('env', String(values.env ?? 'dev')) as EnvironmentName;
  ...
  const containered = str2bool(getArgValue('containered', String(values.containered ?? 'true')));
...
`);
  ...
  if (!containered && overwriteBitcoinconf) {
    ...
    if (importKeysFlag) {
      await promptEnter('Start bitcoin core, and press Enter to import key import...');
    }
  }
