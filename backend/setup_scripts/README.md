...
- or on a local dev machine, where each backend service can be run through 'uv', and redis/postgres servers are ran on localhost too. This should be mainly used as a dev setup for debugging with VS code.
...
`uv run src/main.py -env=prod -containered=false -generate-keys -import-keys-to-bitcoin-core`
...
`uv run src/main.py -env=prod -containered=true -generate-keys -import-keys-to-bitcoin-core`
