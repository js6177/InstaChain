...
export class DiscordOAuthManager {
    ...
    async getDiscordAccessToken(code: string): Promise<string> {
        try {
            console.log(`[DiscordOAuthManager] Exchanging code for access token. Code: ${code}`);
    ...
    async getDiscordUserInfo(accessToken: string, mongoDb: DatabaseInterface): Promise<[OAuthUser, UserKeys]> {
        try {
            console.log(`[DiscordOAuthManager] Fetching user info with access token.`);
            const response = await axios.get<DiscordUserInfo>(DISCORD_USER_URL, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            });
            console.log(`[DiscordOAuthManager] User info response data:`, JSON.stringify(response.data, null, 2));
    ...
    buildOAuthUser(discordUserInfo: DiscordUserInfo): OAuthUser {
        ...
        user.profile_url = standardizeProfileUrl(`https://discord.com/users/${discordUserInfo.id}`);
