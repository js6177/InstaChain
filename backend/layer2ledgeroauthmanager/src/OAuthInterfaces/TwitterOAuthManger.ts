...
export class TwitterOAuthManager {
  ...
  async getTwitterUserInfo(accessToken: string, mongoDb: DatabaseInterface): Promise<[OAuthUser, UserKeys]> {
    ...
    try {
      ...
      const twitterUserInfoData: TwitterUserInfoData = response.data as TwitterUserInfoData;
