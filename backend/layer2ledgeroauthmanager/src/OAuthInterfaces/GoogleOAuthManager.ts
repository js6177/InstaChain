...
export class GoogleOAuthManager {
    ...
    async getGoogleAccessToken(code: string): Promise<string> {
        try {
            ...
            console.error(`[GoogleOAuthManager] Error exchanging code for token:`, axios.isAxiosError(error) ? (error.response?.data || error.message) : error);
            throw error;
        }
    }
    ...
    async getGoogleUserInfo(accessToken: string, mongoDb: DatabaseInterface): Promise<[OAuthUser, UserKeys]> {
        try {
            ...
            console.error(`[GoogleOAuthManager] Error fetching user info:`, axios.isAxiosError(error) ? (error.response?.data || error.message) : error);
            throw error;
        }
    }
