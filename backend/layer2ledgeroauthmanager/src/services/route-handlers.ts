...
function buildFindOAuthUserErrorResponse(
	code: ErrorCode,
...
function buildFindOauth2UserByIdErrorResponse(
	code: ErrorCode,
...
export function createOAuthRouteHandlers(
	...
	return {
		...
		async exchange(
			...
			let accessToken = "";
			if (
				body.service === OAuthService.Twitter &&
				twitterOAuthManager &&
				body.code_verifier
			) {
				try {
					accessToken = await twitterOAuthManager.getTwitterAccessToken(
						body.code,
						body.code_verifier,
					);
					const [userInfo, userKeys] =
						await twitterOAuthManager.getTwitterUserInfo(accessToken, mongoDb);
					return {
				...
				try {
					accessToken = await githubOAuthManager.getGithubAccessToken(
						body.code,
					);
					const [userInfo, userKeys] =
						await githubOAuthManager.getGithubUserInfo(accessToken, mongoDb);
					return {
						error_response: buildErrorResponse(ErrorCodes.Success),
						user: userInfo,
						user_keys: userKeys,
				...
				try {
					accessToken = await googleOAuthManager.getGoogleAccessToken(
						body.code,
					);
					const [userInfo, userKeys] =
						await googleOAuthManager.getGoogleUserInfo(accessToken, mongoDb);
					return {
						error_response: buildErrorResponse(ErrorCodes.Success),
						user: userInfo,
						user_keys: userKeys,
				...
				body.service === OAuthService.Facebook &&
				facebookOAuthManager
			) {
				try {
					accessToken = await facebookOAuthManager.getFacebookAccessToken(
						body.code,
					);
					const [userInfo, userKeys] =
						await facebookOAuthManager.getFacebookUserInfo(
							accessToken,
							mongoDb,
						);
					return {
				...
				try {
					accessToken = await discordOAuthManager.getDiscordAccessToken(
						body.code,
					);
					const [userInfo, userKeys] =
						await discordOAuthManager.getDiscordUserInfo(accessToken, mongoDb);
					return {
						error_response: buildErrorResponse(ErrorCodes.Success),
						user: userInfo,
						user_keys: userKeys,
		...
		async authorizeWithLayer2Token(
			...
			try {
				const [user, user_keys] =
					await mongoDb.authorizeOAuthUserWithLayer2Token(
		...
		async findUserById(
			...
			try {
				const user = await mongoDb.getOAuthUser(
					body.service_name,
					body.service_specific_id,
				);
