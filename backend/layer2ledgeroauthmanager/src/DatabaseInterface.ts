...
export class DatabaseInterface {
		...
		try {
			...
			if (userKeys) {
				if (!userKeys.l2_address_public_key) {
					userKeys.l2_address_public_key = createLayer2AddressPubkey(
						userKeys.l2_address_mneumonic,
					);
					await userKeys.save();
				}
				return userKeys;
	...
	async authorizeOAuthUserWithLayer2Token(
		layer2Token: Layer2OAuthToken,
	): Promise<[OAuthUser | null, UserKeys | null]> {
		const user = await OAuthUserModel.findOne({
			layer2_authorization_token: layer2Token.layer2_authorization_token,
		});
		if (!user) {
			return [null, null];
		}
	...
	async createNewUserKeys(userId: string): Promise<UserKeys> {
		...
		try {
			const existingUserKeys = await UserKeysModel.findOne({
	...
	async findUser(
		username: string | null,
		profile_url: string | null,
		createTemporaryUserIfNotFound: boolean = false,
	): Promise<OAuthUserAuxillaryInfo | null> {
