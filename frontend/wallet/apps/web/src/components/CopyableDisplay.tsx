...
export function CopyableDisplay({
	...
	const handleCopy = (): void => {
		...
		toast.success(
			label
				? `${label.replace(":", "")} copied to clipboard`
				: "Copied to clipboard",
		);
	...
	const [dynamicFontSize, setDynamicFontSize] = useState<string | undefined>(
		undefined,
	);
	...
	if (fitTextInView) {
		if (fitTextInViewMethod === FitTextMethod.MakeFieldMultiline) {
			appliedTextClassName = cn(
				textClassName,
				"whitespace-normal break-all overflow-visible text-clip",
			);
			...
			const stripped = textClassName.replace(
				/\btext-(lg|xl|2xl|3xl|4xl|5xl|base)\b/g,
				"",
			);
		...
		<div className="flex flex-col w-full gap-2 mt-1 mb-1 min-w-0">
			...
			<div className="flex flex-row items-center justify-between gap-4 p-3 bg-muted/20 border rounded-md shadow-sm overflow-hidden min-w-0">
				<div ref={containerRef} className="min-w-0 flex-1 overflow-hidden">
					<p
						className={cn(
							appliedTextClassName,
							`w-full text-foreground transition-all duration-300 ${secret && !isVisible ? "blur-sm select-none" : ""}`,
						)}
						style={customStyle}
					>
