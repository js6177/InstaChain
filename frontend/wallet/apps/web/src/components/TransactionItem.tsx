...
export function TransactionItem({
	...
	const isOutgoing = currentAddress
		? transaction.source_address_pubkey === currentAddress
		: false;
	const isIncoming = currentAddress
		? transaction.destination_address_pubkey === currentAddress
		: false;

	const getTypeString = (type: number): string => {
		switch (type) {
			case TransactionType.TRX_TRANSFER:
				return LABELS.TX_TYPE_TRANSFER;
			case TransactionType.TRX_DEPOSIT:
				return LABELS.TX_TYPE_DEPOSIT;
			case TransactionType.TRX_WITHDRAWAL_INITIATED:
				return LABELS.TX_TYPE_WITHDRAWAL_INITIATED;
			case TransactionType.TRX_WITHDRAWAL_BROADCASTED:
				return LABELS.TX_TYPE_WITHDRAWAL_BROADCASTED;
			case TransactionType.TRX_WITHDRAWAL_CANCELED:
				return LABELS.TX_TYPE_WITHDRAWAL_CANCELED;
			case TransactionType.TRX_WITHDRAWAL_CONFIRMED:
				return LABELS.TX_TYPE_WITHDRAWAL_CONFIRMED;
			default:
				return LABELS.TX_TYPE_UNKNOWN;
		}
	};
	...
	const amountStr = isOutgoing
		? `-${formattedAmount} ${denomination}`
		: isIncoming
			? `+${formattedAmount} ${denomination}`
			: `${formattedAmount} ${denomination}`;
	...
	return (
		<AccordionItem
			value={transaction.layer2_transaction_id}
			className="border bg-card rounded-md px-4 mb-2"
		>
				...
				<div className="flex items-center justify-between w-full text-left pr-4">
					<div className="flex items-center gap-3">
						...
						<div className="overflow-hidden">
							<p
								className="font-semibold text-sm w-32 truncate"
								title={transaction.layer2_transaction_id}
								data-testid={TEST_IDS.TRANSACTION_TITLE}
							>
								<Link
									to={ROUTES.buildExplorerTransaction(
					...
					<div className="flex flex-col items-end gap-1">
						<span
							className={`font-mono font-bold ${isOutgoing ? "text-red-500" : isIncoming ? "text-green-500" : ""}`}
						>
							{amountStr}
						</span>
						<Badge variant="outline" className="text-[10px] h-4 leading-3 px-1">
							{getTypeString(transaction.transaction_type)}
						</Badge>
					</div>
				</div>
			</AccordionTrigger>
			<AccordionContent className="text-sm border-t pt-3 pb-4">
				<div className="space-y-2 font-mono text-xs">
					...
					<div className="grid grid-cols-[120px_1fr] items-start">
						...
						<span>
							{formatAmount(transaction.amount, denomination)} {denomination}
						</span>
					</div>
					<div className="grid grid-cols-[120px_1fr] items-start">
						<span className="text-muted-foreground">{LABELS.KEY_FEE}</span>
						<span>
							{formatAmount(transaction.fee, denomination)} {denomination}
						</span>
					</div>
					{transaction.transaction_type === TransactionType.TRX_TRANSFER ? (
						<>
							<div className="grid grid-cols-[120px_1fr] items-start">
								...
								<Link
									to={ROUTES.buildExplorerAddress(
						...
						<div className="grid grid-cols-[120px_1fr] items-start">
							<span className="text-muted-foreground">
								{LABELS.KEY_LAYER1_TXID}
							</span>
							<span className="break-all text-foreground">
								{transaction.layer1_transaction_id || LABELS.VALUE_PENDING}
							</span>
						</div>
					)}
					<div className="grid grid-cols-[120px_1fr] items-start">
