...
test("wallet is restored from mnemonic", async () => {
	...
	expect(secondWallet.addresses[0].public_key_str_base58).toBe(
		address.public_key_str_base58,
	);
	expect(secondWallet.addresses[0].private_key_str_base58).toBe(
		address.private_key_str_base58,
	);
...
test("wallet store is restored from mnemonic", async () => {
	...
	expect(state.mainAddress?.public_key_str_base58).toBe(
		state.wallet?.addresses[0].public_key_str_base58,
	);
...
test("UI: wallet restoration through the UI", async () => {
	...
	expect(useWalletStore.getState().wallet?.mnemonic).toEqual(
		testKeys.mnemonicWords,
	);
});
