...
describeIntegration('Ledger API functional integration', () => {
  ...
  it('transfer updates balances and transaction history for both addresses', async () => {
    ...
    await seedWalletAddress(sourceAddress.public_key_str_base58, seedBalance, false)
    ...
    await waitForBalance(destinationAddress.public_key_str_base58, transferAmount)

    const sourceBalance = await getAddressBalance(sourceAddress.public_key_str_base58)
  ...
  it('withdrawal debits address balance and records a withdrawal transaction', async () => {
    ...
    const balance = await getAddressBalance(sourceAddress.public_key_str_base58)
    expect(balance).toBe(seedBalance - withdrawAmount)

    const transactions = await getAddressTransactions(sourceAddress.public_key_str_base58)
    expectWithdrawalTransaction(
      transactions,
      withdrawAmount,
      sourceAddress.public_key_str_base58,
