...
export class BitcoinRPCClient {
	...
	private async callRaw<T>(
		...
		if (!response.ok && body.error === null) {
			throw new Error(
				`Bitcoin RPC HTTP ${response.status}: ${JSON.stringify(body)}`,
			);
		}
		return body;
	}
	...
	private async call<T>(method: string, params: unknown[] = []): Promise<T> {
		...
		if (response.error) {
			throw new Error(
				`Bitcoin RPC error: ${response.error.code} - ${response.error.message}`,
			);
		}
	...
	async loadWallet(
		filename: string,
	): Promise<BitcoinRpcResponse<LoadWalletResult>> {
	...
	async importDescriptors(
		...
		return await this.call<ImportDescriptorResult[]>("importdescriptors", [
			requests,
		]);
	}
	...
	async getBlockHeader(
		...
		return (await this.call("getblockheader", [
			blockHash,
			verbose,
		])) as GetBlockHeaderResult;
	}

	async listAddressGroupings(): Promise<ListAddressGroupingsResult> {
		const result = await this.call<Array<Array<[string, number, string?]>>>(
