...
describe.skipIf(!runIntegration)(
	"deposit address derivation vs bitcoin-core",
	() => {
		...
		it("derived deposit addresses match successive getnewaddress results", async () => {
			...
			const descriptors = generateBitcoinCoreDescriptorSegwit(
				masterKeys.master_xprv,
				testnet,
			);
			...
			for (const [index, result] of importResults.entries()) {
				expect(
					result.success,
					`descriptor ${index} import failed: ${JSON.stringify(result.error)}`,
				).toBe(true);
			}
