...
INSTRUCTION_LAYER1_AUDIT = 8 # instruction to get a withdrawal address

DEFAULT_ELLIPTICAL_CURVE = ecdsa.SECP256k1
DEFAULT_KEY_HASH_FUNCTION = hashlib.sha256
NODE_ID = 'BbwLnyLR9eVjL2qc'
...
def signLayer1AuditReportMessage(onboarding_signing_private_key, blockHeight, balance):
    message = NODE_ID + " " + str(INSTRUCTION_LAYER1_AUDIT) + ' ' + str(blockHeight) + ' ' + str(balance)
    return sign_string(onboarding_signing_private_key, message)
