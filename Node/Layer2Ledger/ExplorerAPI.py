...
class search(InstachainRequestHandler):
    def getParameters(self):
        ...
        GlobalLogging.logger.log_text("v1 search_string: " + self.search_string)
        GlobalLogging.logger.log_text("v1 search_type: " + self.search_type)
    def processRequest(self):
        GlobalLogging.logger.log_text("starting search")
        ...
        if(self.search_type not in [member.value for member in SearchType]):
            ...
            GlobalLogging.logger.log_text("search_type: not supported")
            return
        if((self.search_type == SearchType.L2_TRANSACTION) or (self.search_type == SearchType.SEARCH_ALL.value)):
            GlobalLogging.logger.log_text("search... L2_TRANSACTION")
            (error_code, transaction) = Transaction.get_transaction(self.search_string)
            GlobalLogging.logger.log_text("search... L2_TRANSACTION found : " + str(error_code))
